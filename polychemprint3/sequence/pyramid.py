...
class pyramid(sequenceSpec):
    ...
    def genSequence(self):
        ...
        try:
            ...
            count = float(step)
            LReduction = (float(length) / (float(height) / float(step) - 1)) / 2

            WReduction = (float(width) / (float(height) / float(step) - 1)) / 2
            LMove = float(length)
            WMove = float(width)

            # other layers
            while count < float(height):
                LMove = LMove - 2 * LReduction

                WMove = WMove - 2 * WReduction
                cmds.append(("axes.move(\"G1 F" + str(printSpd)
                             + " X" + str(LReduction) + "\\n" + "\")"))
                cmds.append(("axes.move(\"G1 F" + str(printSpd)
                             + " X" + str(WReduction) + "\\n" + "\")"))
