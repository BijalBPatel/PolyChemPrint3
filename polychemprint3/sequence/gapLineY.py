...
class gapLineY(sequenceSpec):
    ...
    def __init__(self, axes: axes3DSpec = nullAxes(), tool: toolSpec = nullTool(), **kwargs):
        ...
        # Create Params dict
        self.dictParams = {
            "name": seqParam("Sequence Name", "gapLineY", "",
                             "Change if modifying from default"),
            "description": seqParam("Sequence Description",
                                    "Lines in Y-direction with a gap where tool raises", "", "gapLineY.py"),
            "creationDate": seqParam("Creation Date",
                                     "05/17/2021", "", "dd/mm/yyyy"),
            "createdBy": seqParam("Created By", "Giorgio Graziano", "", ""),
            "owner": seqParam("Owner", "PCP_Electronics", "",
                              ""),
            "printSpd": seqParam("Printing Speed", "60", "mm/min", ""),
            "trvlSpd": seqParam("Travel Speed", "200", "mm/min", ""),
            "ySegLength": seqParam("y-seglength", "10", "mm", "Length of each Y segment"),
            "yGap": seqParam("Y-Gap", "2", "mm", "Length of gap in Y"),
            "numRows": seqParam("numRows", "3", "mm", "Number of lines to print"),
            "xSpacing": seqParam("X-Spacing", "3", "mm", "Spacing (in x) between lines"),
    ...
    def genSequence(self):
        ...
        try:
            ...
            ySegLength = self.dictParams.get("ySegLength").value
            yGap = self.dictParams.get("yGap").value
            xSpacing = self.dictParams.get("xSpacing").value
            ...
            # Calculated Values
            totalY = 2 * float(ySegLength) + float(yGap)
